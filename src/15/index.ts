export default function drawTable(data: Array<Record<string, string | number>>): string {
    ...
    const columnWidths = headers.map((header) => Math.max(header.length, ...data.map((row) => `${row[header]}`.length)))
    ...
    const rows = data.map((row) => `| ${headers.map((key, i) => `${row[key]}`.padEnd(columnWidths[i])).join(' | ')} |`)
